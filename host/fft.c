...
complex* DFT_naive(complex* x, int N) {
    complex* X = (complex*) calloc( N, sizeof(struct complex_t));
    int k, n;
    for(k = 0; k < N; k++) {
        X[k].re = 0.0;
        X[k].im = 0.0;
...
complex* FFT_GoodThomas(complex* input, int N, int N1, int N2) {
    ...
    complex** columns = (complex**) calloc(N1,sizeof(struct complex_t*));
    for(k1 = 0; k1 < N1; k1++) {
        columns[k1] = (complex*) calloc(N2,sizeof(struct complex_t));
    }
    
    /* Allocate rowwise matrix */
    complex** rows = (complex**) calloc( N2, sizeof(struct complex_t*));
    for(k2 = 0; k2 < N2; k2++) {
        rows[k2] = (complex*) calloc(N1,sizeof(struct complex_t));
    }
    ...
    complex* output = (complex*) calloc( N, sizeof(struct complex_t));
...
void FFT2(complex **inout, int height, int width){
	complex *rows;
	complex *cols = (complex*) calloc(height, sizeof(complex));
	for (int i = 0; i != height; i++) {
		
		rows = FFT_CooleyTukey(inout[i], width, width, 1);
		free(inout[i]);
		inout[i] = rows;
	}
	for (int j = 0; j != width; j++) {
		...
		rows = FFT_CooleyTukey(cols, height, height, 1);
...
void IFFT2(complex **inout, int height, int width) {
	for(int i = 0; i != height; i ++)
		for (int j = 0; j != width; j ++)
			inout[i][j].im = -inout[i][j].im;
	FFT2(inout, height, width);
